...
class Vision:
    ...
    def __init__(self, needle_img_path, method=cv.TM_CCOEFF_NORMED):
        ...
        # Removido cv.IMREAD_UNCHANGED pq tava quebrando
    ...
    def find(self, haystack_img, threshold=0.5, debug_mode=None):
        ...
        if len(rectangles):
            ...
            for (x, y, w, h) in rectangles:
                ...
                # hwnd = dcObj.CreateCompatibleDC()
                monitor = (0, 0, GetSystemMetrics(0), GetSystemMetrics(1))
                hPen = win32gui.CreatePen(win32con.PS_SOLID, 3, win32api.RGB(255, 0, 0))  # Define box color
                win32gui.SelectObject(dc, hPen)
                hbrush = win32gui.GetStockObject(win32con.NULL_BRUSH)

                prebrush = win32gui.SelectObject(dc, hbrush)
                dcObj.Rectangle((top_left[0] + 30, top_left[1] + 40, top_left[0] + 120, top_left[1] + 160))
